...
def get_line(x, y, name, color='#7600fe', isFilled=False, fillcolor='transparent', width=2, showlegend=True):
    if isFilled:
        fill = 'tonexty'
    else:
        fill = 'none'
...
def plot_dist(dist, dist_name='dist', color='#7600fe', range=(0, 20)):
    win = 'dist'
    title = 'Distributions'
...
def plot_loss(epoch, loss, color='#c32b00'):
    win = 'loss'
    title ='Loss'
...
def plot_reward(t, r, color='#00e7aa'):
    win = 'reward'
    title ='Episodic Reward'

    if 'reward' not in d:
        d['reward'] = []
    d['reward'].append((epoch, loss.item()))
    x, y = zip(*d['reward'])
